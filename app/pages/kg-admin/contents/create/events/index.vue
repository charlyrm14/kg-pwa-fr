<script setup lang="ts">
    ...
    import { newEventSchema } from '~/validations/contents/new-event.schema';
    ...
    const eventContent = ref<boolean>(true)
    const imageContent = ref<boolean>(true) 

    const { defineField,  handleSubmit,  errors} = useForm<CreateEventContent>({
        initialValues: {
            content_status_id: 5
        },
    ...
    const onSubmit = handleSubmit(async values => {
        ...
        const payload: CreateEventContent = {
            ...values,
            content_type_id: 2,
            author_id: 2,
            start_date: values.start_date.replace('T', ' '),
            end_date: values.end_date.replace('T', ' ')
        }
        ...
        try {
            ...
            console.error('Error to create event')
        }
    })
    ...
    <section class="md:px-5">
        ...
        <section>
            <Breadcrumb
                ...
                secondary-section="Crear Evento"/>
        </section>
                        ...
                        <div class="bg-white dark:bg-dark border border-gray-200 dark:border-dark-light flex flex-col-reverse md:flex-col rounded-lg">
                            ...
                            <div class="border-b border-gray-200 dark:border-dark-light p-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <button class="text-gray-500 dark:text-white cursor-pointer hover:opacity-75" @click="eventContent = !eventContent">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down-icon " :class="eventContent ? 'lucide-chevron-up' : 'lucide-chevron-down'"><path :d="eventContent ? 'm18 15-6-6-6 6' : 'm6 9 6 6 6-6'"/></svg>
                                    </button>
                                </div>
                                <div v-if="eventContent">
                                    <div class="mt-3">
                                        <label for="start_date" class="text-gray-500 dark:text-white"> Fecha de inicio de evento </label>
                                        <input 
                                            type="datetime-local"
                                            id="start_date"
                                            name="start_date"
                                            v-model="start_date"
                                            v-bind="start_dateAttrs"
                                            class="w-full mt-3 border border-gray-300 dark:border-dark-light text-gray-500 dark:text-white p-4 rounded-lg focus:outline-none font-bold mb-1">
                                        <span v-if="errors.start_date" class="text-red-500 text-sm px-2 font-bold"> 
                                            {{ errors.start_date  }}
                                        </span>
                                    </div>
                                    <div class="mt-3">
                                        <label for="end_date" class="text-gray-500 dark:text-white"> Fecha fin de evento </label>
                                        <input 
                                            type="datetime-local"
                                            id="end_date"
                                            name="end_date"
                                            v-model="end_date"
                                            v-bind="end_dateAttrs"
                                            class="w-full mt-3 border border-gray-300 dark:border-dark-light text-gray-500 dark:text-white p-4 rounded-lg focus:outline-none font-bold mb-1">
                                        <span v-if="errors.end_date" class="text-red-500 text-sm px-2 font-bold"> 
                                            {{ errors.end_date  }}
                                        </span>
                                    </div>
                                    <div class="mt-3">
                                        <label for="location" class="text-gray-500 dark:text-white"> Ubicación </label>
                                        <input 
                                            type="text"
                                            id="location"
                                            name="location"
                                        ...
                                        <span v-if="errors.location" class="text-red-500 text-sm px-2 font-bold"> 
                                            {{ errors.location  }}
                                        </span>
                                    </div>
                                </div>
