<script setup lang="ts">
    ...
    const total = computed(() => attendanceStore?.attendancesToday?.data?.length ?? 0)
</script>

<template>
    <div class="flex justify-center md:justify-between items-center mt-6 flex-wrap gap-4 p-4">
        <div>
            <span class="dark:text-gray-300"> Por página <span class="font-extrabold"> {{ total }} </span> </span>
        </div>
        <div class="flex justify-center md:justify-between items-center gap-x-2 sm:gap-x-4">
            ...
            <button
                class="px-4 py-2 rounded-lg cursor-pointer hover:opacity-75 bg-indigo-500 text-white flex items-center gap-x-2 font-extrabold">
                    Siguiente
