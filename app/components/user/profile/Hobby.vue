...
<template>
    ...
    <div class="bg-white dark:bg-dark-extralight border border-gray-200 dark:border-none rounded-lg p-4 mt-4 shadow flex flex-row-reverse justify-between items-start">
        <div class="flex justify-end items-center">
            <button 
                @click="open('AddHobbiesModal', { userHobbies: user?.hobbies })"
        ...
        <div class="flex flex-wrap gap-2 mt-3">
            <span
                ...
                :key="hobby.id"
                class="bg-sky-500 text-white text-sm md:text-base hover:opacity-75 px-4 py-1 rounded-full whitespace-nowrap font-bold"> 
                    {{ hobby.name ?? 'unknown' }} 
            </span>
            <span 
                v-else
